...
export function StickyBuyBar({ outboundUrl, title, priceRange, isAffiliate, provider }: StickyBuyBarProps) {
  ...
  return (
    <div
      aria-hidden={!visible}
      className={`fixed bottom-0 left-0 right-0 z-40 sm:hidden max-w-full overflow-hidden bg-background/97 backdrop-blur-md border-t border-border/80 px-4 py-3 flex items-center gap-3 shadow-[0_-8px_30px_rgba(0,0,0,0.4)] transition-transform duration-300 ${
    ...
    >
      ...
      <a
        ...
        className={`shrink-0 min-w-0 max-w-[52vw] px-4 py-2.5 rounded-xl text-sm font-bold transition-all active:scale-95 flex items-center justify-center gap-1.5 ${
          isAmazon
      ...
      >
        ...
        <span className="truncate">{isAmazon ? "Check Price" : "View Now"}</span>
      </a>
    </div>
  );
}
