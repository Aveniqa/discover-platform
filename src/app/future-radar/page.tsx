...
export const metadata: Metadata = buildMetadata({
  title: "Future Radar — Archive",
  description:
    "An archive of emerging-technology briefs published on Surfaced. The site now focuses on software and tools — these older entries remain online.",
  path: "/future-radar",
});

export default function FutureRadarArchivePage() {
  const items = (archiveData as AnyItem[])
    .filter((i) => i.type === "future-tech")
  ...
  return (
    <article>
      <section className="relative min-h-[55vh] flex items-center overflow-hidden">
        <AlcoveBackdrop alcove={alcoveByKind("ai")} trackScroll />
        <div className="relative z-10 max-w-3xl mx-auto px-4 sm:px-6 py-24 sm:py-32">
          ...
          <h1 className="text-4xl sm:text-6xl font-bold text-white tracking-tight leading-tight">
            The future-tech archive.
          </h1>
          <p className="mt-5 text-white/85 text-lg leading-relaxed">
            Surfaced shifted from emerging-tech speculation to hand-edited software coverage. These
            briefs stay online at their original URLs.
          </p>
          <div className="mt-7 flex flex-wrap gap-3">
            <Link href="/tools" className="inline-flex items-center gap-1.5 px-5 py-2.5 rounded-xl bg-white text-black text-sm font-semibold hover:bg-white/90 transition-colors">
              See what Surfaced covers now →
            </Link>
            <Link href="/" className="inline-flex items-center gap-1.5 px-5 py-2.5 rounded-xl bg-white/10 backdrop-blur-md text-white border border-white/20 text-sm font-medium hover:bg-white/20 transition-colors">
      ...
      <section className="bg-background py-20 px-4 sm:px-6">
        <div className="max-w-6xl mx-auto">
          <p className="text-sm text-muted-foreground mb-8">
            Showing the {items.length} most-recent of {(archiveData as AnyItem[]).filter((i) => i.type === "future-tech").length} archived future-tech briefs.
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {items.map((item) => (
              <Link key={item.slug} href={`/item/${item.slug}`} className="group block rounded-2xl border border-border bg-card overflow-hidden hover:border-accent/40 transition-all">
                ...
                <div className="p-5">
                  <p className="text-[10px] uppercase tracking-wider text-muted-foreground mb-2">Future Tech</p>
